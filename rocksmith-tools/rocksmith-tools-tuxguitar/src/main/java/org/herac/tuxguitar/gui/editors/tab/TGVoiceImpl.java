...
public class TGVoiceImpl extends TGVoice {
	...
	public void updateNoteVoice(ViewLayout layout) {
		...
		if (this.previous != null && !this.previous.isRestVoice()) {
			if (getMeasureImpl().canJoin(layout.getSongManager(), this,
					this.previous)) {
				withPrev = true;
				if (this.previous.getDuration().getValue() >= getDuration()
						.getValue()) {
		...
		if (this.next != null && !this.next.isRestVoice()) {
			if (getMeasureImpl().canJoin(layout.getSongManager(), this,
					this.next)) {
				if (this.next.getDuration().getValue() >= getDuration()
						.getValue()) {
		...
		if ((layout.getStyle() & ViewLayout.DISPLAY_TABLATURE) != 0
			...
			this.minY = 0;
			this.maxY = getBeatImpl().getMeasureImpl().getTrackImpl()
					.getTabHeight();
	...
	public void updateSilenceSpacing(ViewLayout layout) {
		...
		if (!isHiddenSilence()) {
			...
			if (duration == TGDuration.WHOLE) {
				...
				this.silenceHeight = (scale * 3.6513581f);
				this.silenceY = (lineCount <= 5 ? lineSpacing * 2
						: lineSpacing * 3) - this.silenceHeight;
				...
				if (v != getIndex()) {
					...
					if (!voice.isEmpty()) {
						if (voice.isRestVoice()) {
							...
							int y1 = voice.getBeatGroup().getY1(layout,
									voice.getMinNote(),
									getMeasureImpl().getKeySignature(),
									getMeasureImpl().getClef());
							int y2 = voice.getBeatGroup().getY1(layout,
									voice.getMaxNote(),
									getMeasureImpl().getKeySignature(),
									getMeasureImpl().getClef());
							...
							int direction = voice.getBeatGroup().getDirection();
							if (direction == TGBeatGroup.DIRECTION_UP) {
								float position = (lineSpacing * voice
										.getMaxString());
								if (position > this.silenceY) {
									this.silenceY = position;
								}
								...
								float position = ((lineSpacing * (voice
										.getMinString() - 1)) - (this.silenceHeight + lineSpacing));
								if (position < this.silenceY) {
									this.silenceY = position;
								}
							}
	...
	public void paintSilence(ViewLayout layout, TGPainter painter, int fromX,
		...
		setStyle(layout, painter,
				(layout.isPlayModeEnabled() && isPlaying(layout)));
		...
		if (!getDuration().getDivision().isEqual(TGDivisionType.NORMAL)) {
			...
			if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
				painter.drawString(
						Integer.toString(getDuration().getDivision()
								.getEnters()),
						x,
		...
		if (!isRestVoice()) {
			...
			if ((style & ViewLayout.DISPLAY_TABLATURE) != 0
				...
				paintTablatureBeat(layout, painter, fromX, fromY
						+ getPaintPosition(TGTrackSpacing.POSITION_TABLATURE),
						spacing);
			}
		}
	}
	...
	public void paintScoreBeat(ViewLayout layout, TGPainter painter, int fromX,
		...
		if (getDuration().getValue() >= TGDuration.HALF) {
			...
			int yMove = (direction == TGBeatGroup.DIRECTION_UP ? ((layout
					.getScoreLineSpacing() / 3) + 1) : ((layout
					.getScoreLineSpacing() / 3) * 2));
			...
			if (getDuration().getValue() >= TGDuration.EIGHTH) {
				...
				if (index >= 0) {
					int dir = (direction == TGBeatGroup.DIRECTION_DOWN) ? 1
							: -1;
					int joinedType = getJoinedType();
					...
					if ((joinedType == TGVoiceImpl.JOINED_TYPE_NONE_LEFT || joinedType == TGVoiceImpl.JOINED_TYPE_NONE_RIGHT)
						...
						float hY = ((fromY + this.group.getY2(layout, getPosX()
								+ spacing, key, clef)) - ((lineSpacing * 2) * dir));
						for (int i = 0; i <= index; i++) {
							painter.initPath(TGPainter.PATH_FILL);
							TGNotePainter.paintFooter(painter, hX,
									(hY - ((i * (lineSpacing / 2.0f)) * dir)),
									dir, lineSpacing);
							painter.closePath();
						}
					} else {
						...
						int hY1 = fromY
								+ this.group.getY2(layout, hX1, key, clef);
						int hY2 = fromY
								+ this.group.getY2(layout, hX2, key, clef);
	...
	public void paintDot(ViewLayout layout, TGPainter painter, float fromX,
		...
		painter.addOval(posX - (dotSize / 2), posY - (dotSize / 2), dotSize,
				dotSize);
		if (getDuration().isDoubleDotted()) {
			painter.moveTo(posX + (dotSize + 2) - (dotSize / 2), posY
					- (dotSize / 2));
	...
	public String toString() {
		return "TGVoiceImpl [getIndex()=" + getIndex() + ", isEmpty()="
				+ isEmpty() + ", getDirection()=" + getDirection()
				+ ", getDuration()=" + getDuration() + ", isRestVoice()="
				+ isRestVoice() + "]";
	}
}

...
public abstract class TGDuration {
	...
	public static TGDuration fromTime(TGFactory factory, long time,
		...
		while (!finish) {
			...
			if (tmpTime - diff <= time) {
				...
				if (Math.abs(tmpTime - time) < Math.abs(duration.getTime()
						- time)) {
	...
	public boolean isEqual(TGDuration d) {
		return (getValue() == d.getValue() && isDotted() == d.isDotted()
				&& isDoubleDotted() == d.isDoubleDotted() && getDivision()
				.isEqual(d.getDivision()));
	}
	...
	public String toString() {
		return "TGDuration [getValue()=" + getValue() + ", isDotted()="
				+ isDotted() + ", isDoubleDotted()=" + isDoubleDotted()
				+ ", getTime()=" + getTime() + ", getIndex()=" + getIndex()
				+ "]";
	}

...
public class TGBeatImpl extends TGBeat {
	...
	public boolean[] getUsedStrings() {
		if (this.usedStrings == null) {
			this.usedStrings = new boolean[getMeasure().getTrack()
					.stringCount()];
		}
	...
	public TGVoiceImpl getVoiceImpl(int index) {
		...
		if (voice instanceof TGVoiceImpl) { return (TGVoiceImpl) voice; }
		return null;
	}
	...
	public int getEffectsSpacing(ViewLayout layout) {
		if (this.accentuated) {
			this.bs.setSize(TGBeatSpacing.POSITION_ACCENTUATED_EFFECT,
					layout.getEffectSpacing());
		}
		if (this.heavyAccentuated) {
			this.bs.setSize(TGBeatSpacing.POSITION_HEAVY_ACCENTUATED_EFFECT,
					layout.getEffectSpacing());
		}
		if (this.harmonic) {
			this.bs.setSize(TGBeatSpacing.POSITION_HARMONIC_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.tapping) {
			this.bs.setSize(TGBeatSpacing.POSITION_TAPPING_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.slapping) {
			this.bs.setSize(TGBeatSpacing.POSITION_SLAPPING_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.popping) {
			this.bs.setSize(TGBeatSpacing.POSITION_POPPING_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.palmMute) {
			this.bs.setSize(TGBeatSpacing.POSITION_PALM_MUTE_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.fadeIn) {
			this.bs.setSize(TGBeatSpacing.POSITION_FADE_IN,
					layout.getEffectSpacing());
		}
		if (this.vibrato) {
			this.bs.setSize(TGBeatSpacing.POSITION_VIBRATO_EFFEC,
					layout.getEffectSpacing());
		}
		if (this.trill) {
			this.bs.setSize(TGBeatSpacing.POSITION_TRILL_EFFEC,
					layout.getEffectSpacing());
		}
			...
			new Thread(new Runnable() {
				public void run() {
					...
					TuxGuitar.instance().getPlayer()
							.playBeat(getMeasure().getTrack(), notes);
				}
			}).start();
		}
	}
	...
	public void paintExtraLines(TGPainter painter, ViewLayout layout,
			int fromX, int fromY) {
		if (!isRestBeat()) {
	...
	public String toString() {
		return "TGBeatImpl [posX=" + posX + ", width=" + width + ", maxNote="
				+ maxNote + ", minNote=" + minNote + ", usedStrings="
				+ Arrays.toString(usedStrings) + ", joinedType=" + joinedType
				+ ", joinedGreaterThanQuarter=" + joinedGreaterThanQuarter
				+ ", join1=" + join1 + ", join2=" + join2 + ", previous="
				+ previous + ", next=" + next + ", group=" + group + ", bs="
				+ bs + ", accentuated=" + accentuated + ", heavyAccentuated="
				+ heavyAccentuated + ", harmonic=" + harmonic + ", tapping="
				+ tapping + ", slapping=" + slapping + ", popping=" + popping
				+ ", palmMute=" + palmMute + ", vibrato=" + vibrato
				+ ", trill=" + trill + ", fadeIn=" + fadeIn + ", getChord()="
				+ getChord() + ", getText()=" + getText() + ", isChordBeat()="
				+ isChordBeat() + ", isTextBeat()=" + isTextBeat() + "]";
	}
}

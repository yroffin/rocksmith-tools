...
public class TGMeasureImpl extends TGMeasure {
	...
	public static final int KEY_SIGNATURES[][] = new int[][] {
			...
			{ NATURAL, NATURAL, NATURAL, SHARP, NATURAL, NATURAL, NATURAL }, // 1
																				// SHARP
			{ SHARP, NATURAL, NATURAL, SHARP, NATURAL, NATURAL, NATURAL }, // 2
																			// SHARPS
			{ SHARP, NATURAL, NATURAL, SHARP, SHARP, NATURAL, NATURAL }, // 3
																			// SHARPS
			{ SHARP, SHARP, NATURAL, SHARP, SHARP, NATURAL, NATURAL }, // 4
																		// SHARPS
			...
			// ------------FLATS------------------------------------
			{ NATURAL, NATURAL, NATURAL, NATURAL, NATURAL, NATURAL, FLAT }, // 1
																			// FLAT
			{ NATURAL, NATURAL, FLAT, NATURAL, NATURAL, NATURAL, FLAT }, // 2
																			// FLATS
	...
	public static final int ACCIDENTAL_SHARP_NOTES[] = new int[] { 0, 0, 1, 1,
			2, 3, 3, 4, 4, 5, 5, 6 };
	public static final int ACCIDENTAL_FLAT_NOTES[] = new int[] { 0, 1, 1, 2,
			2, 3, 4, 4, 5, 5, 6, 6 };
	public static final boolean ACCIDENTAL_NOTES[] = new boolean[] { false,
	...
	private final List[] voiceGroups;
	...
	private final boolean[][] registeredAccidentals;
	...
	private void checkCompactMode(ViewLayout layout) {
		...
		if (compactMode
				&& (layout.getStyle() & ViewLayout.DISPLAY_MULTITRACK) != 0) {
		...
		for (int i = 0; i < countBeats(); i++) {
			...
			if (chordEnabled && beat.getChord() != null) {
				if (previousChord != null) {
					int length = (int) (beat.getStart() - previousChord
							.getBeat().getStart());
			...
			for (int v = 0; v < TGBeat.MAX_VOICES; v++) {
				...
				if (!voice.isEmpty()) {
					...
					voice.reset();
					if (minDuration == null
							|| voice.getDuration().getTime() <= minDuration
									.getTime()) {
					...
					if (!voice.isRestVoice()) {
						...
						if ((groups[v] == null)
								|| !canJoin(layout.getSongManager(), voice,
										previousVoices[v])) {
						...
						for (int v2 = 0; v2 < TGBeat.MAX_VOICES; v2++) {
							if (v2 != voice.getIndex()) {
								...
								if (!voice2.isEmpty()
										&& voice2.getDuration().isEqual(
												voice.getDuration())) {
									if (!voice2.isRestVoice()
											|| !voice2.isHiddenSilence()) {
			...
			if (emptyBeat) {
				System.out.println("Empty Beat !!!!!! " + beat.getStart()
						+ "  " + i);
			}
		...
		if (!this.compactMode) {
			...
			if (chordEnabled && minimumChordLength > 0) {
				int chordWidth = (layout.getChordFretIndexSpacing()
						+ layout.getChordStringSpacing() + (getTrack()
						.stringCount() * layout.getChordStringSpacing()));
				int minimumSpacing = (int) ((TGDuration.QUARTER_TIME * chordWidth) / minimumChordLength);
				this.quarterSpacing = Math.max(minimumSpacing,
						this.quarterSpacing);
			}
	...
	public boolean canJoin(TGSongManager manager, TGVoiceImpl b1, TGVoiceImpl b2) {
		if (b1 == null || b2 == null || b1.isRestVoice() || b2.isRestVoice()) { return false; }
		...
		if (b1.getDuration().getValue() < TGDuration.EIGHTH
				|| b2.getDuration().getValue() < TGDuration.EIGHTH) { return (start1 == start2); }
	...
	private void makeBeat(ViewLayout layout, TGBeatImpl beat,
		...
		if (previousBeat != null) {
			...
			if (chordEnabled && beat.isChordBeat()
					&& previousBeat.isChordBeat()) {
	...
	public void calculateMeasureChanges(ViewLayout layout) {
		...
		if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0) {
			if (this.prevMeasure == null
					|| getClef() != this.prevMeasure.getClef()) {
				this.paintClef = true;
	...
	private void updateComponents(ViewLayout layout) {
		...
		for (int i = 0; i < countBeats(); i++) {
			...
			if (this.compactMode) {
				...
				int x1 = (spacing + TablatureUtil.getStartPosition(this,
						beat.getStart(), quarterWidth));
				int minimumWidth = -1;
				for (int v = 0; v < beat.countVoices(); v++) {
					TGVoiceImpl voice = beat.getVoiceImpl(v);
					if (!voice.isEmpty()) {
						int x2 = (spacing + TablatureUtil.getStartPosition(
								this, beat.getStart()
										+ voice.getDuration().getTime(),
								quarterWidth));
						int width = (x2 - x1);
			...
			for (int v = 0; v < beat.countVoices(); v++) {
				...
				if (!voice.isEmpty()) {
					...
					if (!this.division
							&& !voice.getDuration().getDivision()
									.isEqual(TGDivisionType.NORMAL)) {
						this.division = true;
					}
					if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) == 0
	...
	public void registerSpacing(ViewLayout layout, TGTrackSpacing ts) {
		if (layout.hasLoopMarker(this)) {
			ts.setSize(TGTrackSpacing.POSITION_LOOP_MARKER,
					layout.getLoopMarkerSpacing());
		}
		if (this.hasMarker()) {
			ts.setSize(TGTrackSpacing.POSITION_MARKER,
					layout.getMarkerSpacing());
		}
		if (this.chord) {
			ts.setSize(TGTrackSpacing.POSITION_CHORD,
					layout.getDefaultChordSpacing());
		}
		if (this.text) {
			ts.setSize(TGTrackSpacing.POSITION_TEXT, layout.getTextSpacing());
		}
		if (this.getHeader().getRepeatAlternative() > 0) {
			ts.setSize(TGTrackSpacing.POSITION_REPEAT_ENDING,
					layout.getRepeatEndingSpacing());
		}
		if (this.division) {
			ts.setSize(TGTrackSpacing.POSITION_DIVISION_TYPE,
					layout.getDivisionTypeSpacing());
		}
	...
	public void paintMeasure(ViewLayout layout, TGPainter painter) {
		...
		if (shouldRepaintBuffer() || !bufferEnabled) {
			...
			if (bufferEnabled) {
				getBuffer().createBuffer(getWidth(layout) + getSpacing(),
						getTs().getSize(),
						layout.getResources().getBackgroundColor());
		...
		if (bufferEnabled) {
			...
			getBuffer().paintBuffer(
					painter,
					getPosX(),
					getPosY(),
					getTs().getPosition(
							TGTrackSpacing.POSITION_BUFFER_SEPARATOR));
		}
	...
	public void paintRepeatEnding(ViewLayout layout, TGPainter painter) {
		if (getHeader().getRepeatAlternative() > 0) {
			...
			painter.closePath();
			painter.drawString(string, Math.round(x1 + (5.0f * scale)),
					Math.round(y1 + (2.0f * scale)));
		}
	}
	 ...
	 * Pinta las notas
	 */
	public void paintComponents(ViewLayout layout, TGPainter painter,
			int fromX, int fromY) {
		Iterator it = getBeats().iterator();
		while (it.hasNext()) {
			TGBeatImpl beat = (TGBeatImpl) it.next();
			beat.paint(layout, painter,
					fromX + getHeaderImpl().getLeftSpacing(layout), fromY);
		}
	}
	...
	private void paintDivisions(ViewLayout layout, TGPainter painter) {
		...
		if ((style & ViewLayout.DISPLAY_TABLATURE) != 0) {
			y1 = getPosY()
					+ getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE);
			y2 = y1
					+ ((getTrack().getStrings().size() - 1) * layout
							.getStringSpacing());
	...
	private void paintDivisions(ViewLayout layout, TGPainter painter, int x1,
		...
		if (addInfo) {
			...
			painter.drawString(
					number,
					getPosX() + Math.round(scale),
					(y1 - painter.getStringExtent(number).y)
							- Math.round(scale));
		}
		...
		if (this.isRepeatOpen() || layout.isFirstMeasure(this)) {
			...
			painter.lineTo(x1 + lineWidthBig + scale + lineWidthSmall,
					(y2 + offsetY));
			painter.closePath();
			...
			if (this.isRepeatOpen()) {
				...
				painter.moveTo(x1 + xMove, y1 + ((y2 - y1) / 2)
						- (yMove + (size / 2)));
				painter.addOval(x1 + xMove, y1 + ((y2 - y1) / 2)
		...
		if (this.getRepeatClose() > 0 || layout.isLastMeasure(this)) {
			...
			if (this.getRepeatClose() > 0) {
				...
				painter.setLineWidth(lineWidthSmall);
				painter.initPath(TGPainter.PATH_FILL);
				painter.moveTo((x2 - xMove) + getSpacing(), y1
						+ ((y2 - y1) / 2) - (yMove + (size / 2)));
				painter.addOval((x2 - xMove) + getSpacing(), y1
						+ ((y2 - y1) / 2) - (yMove + (size / 2)), size, size);
				painter.moveTo((x2 - xMove) + getSpacing(), y1
						+ ((y2 - y1) / 2) + (yMove - (size / 2)));
				painter.addOval((x2 - xMove) + getSpacing(), y1
						+ ((y2 - y1) / 2) + (yMove - (size / 2)), size, size);
				...
				if (addInfo) {
					...
					painter.drawString(repetitions, x2 - numberSize.x
							+ getSpacing() - size,
							(y1 - numberSize.y) - Math.round(scale));
				}
			}
		} else {
			painter.initPath();
	...
	private void paintClef(ViewLayout layout, TGPainter painter, int fromX,
		...
		if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0
			...
			if (this.getClef() == TGMeasure.CLEF_TREBLE) {
				TGClefPainter.paintTreble(painter, x, y,
						layout.getScoreLineSpacing());
			} else if (this.getClef() == TGMeasure.CLEF_BASS) {
				TGClefPainter.paintBass(painter, x, y,
						layout.getScoreLineSpacing());
			} else if (this.getClef() == TGMeasure.CLEF_TENOR) {
				TGClefPainter.paintTenor(painter, x, y,
						layout.getScoreLineSpacing());
			} else if (this.getClef() == TGMeasure.CLEF_ALTO) {
				TGClefPainter.paintAlto(painter, x, y,
						layout.getScoreLineSpacing());
			}
			painter.closePath();
		}
		...
		if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0
			...
			if (currentKey >= 1 && currentKey <= 7) {
				for (int i = 0; i < currentKey; i++) {
					float offset = (((scale / 2) * SCORE_KEY_SHARP_POSITIONS[clefIndex][i]) - (scale / 2));
					painter.initPath(TGPainter.PATH_FILL);
					TGKeySignaturePainter.paintSharp(painter, x, (y + offset),
							scale);
					painter.closePath();
					x += (scale - (scale / 4));
				}
			...
			// flats
				...
				for (int i = 7; i < currentKey; i++) {
					float offset = (((scale / 2) * SCORE_KEY_FLAT_POSITIONS[clefIndex][i - 7]) - (scale / 2));
					painter.initPath(TGPainter.PATH_FILL);
					TGKeySignaturePainter.paintFlat(painter, x, (y + offset),
							scale);
					painter.closePath();
					x += (scale - (scale / 4));
				}
	...
	private void paintTimeSignature(ViewLayout layout, TGPainter painter,
		...
		if (this.getHeaderImpl().shouldPaintTimeSignature()) {
			...
			String numerator = Integer.toString(getTimeSignature()
					.getNumerator());
			String denominator = Integer.toString(getTimeSignature()
					.getDenominator().getValue());
			if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
				int y = getTs().getPosition(
						TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES);
				...
				int y2 = ((y + getTrackImpl().getTabHeight()) - painter
						.getStringExtent(denominator).y) + move;
				painter.drawString(numerator, fromX + x, fromY + y1, true);
				painter.drawString(denominator, fromX + x, fromY + y2, true);
			}
		}
	...
	private void paintTempo(ViewLayout layout, TGPainter painter) {
		if (this.getHeaderImpl().shouldPaintTempo()) {
			...
			int y = getPosY();
			int lineSpacing = (Math.max(layout.getScoreLineSpacing(),
					layout.getStringSpacing()));
			int style = layout.getStyle();
			if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
				y += (getTs().getPosition(
						TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) - lineSpacing);
			} else if ((style & ViewLayout.DISPLAY_TABLATURE) != 0) {
			...
			if (layout.isBufferEnabled()) {
				painter.drawImage(layout.getResources().getTempoImage(), imgX,
						imgY);
			} else {
	...
	private void paintTripletFeel(ViewLayout layout, TGPainter painter) {
		if (this.getHeaderImpl().shouldPaintTripletFeel()) {
			...
			int x1 = x;
			int x2 = x
					+ (Math.round((1.33f * scale) + (1.5f * scale)) + fontSize.x);
			int y1 = y - (Math.round((1.0f * scale) + (2.5f * scale)) + 2);
			int y2 = y
					- (Math.round((1.0f * scale) + (2.5f * scale)
							+ (1.0f * scale)) + 2);
			...
			if (getTripletFeel() == TGMeasureHeader.TRIPLET_FEEL_NONE
				...
				if (previous == TGMeasureHeader.TRIPLET_FEEL_EIGHTH) {
					if (layout.isBufferEnabled()) {
						painter.drawImage(layout.getResources()
								.getTripletFeel8(), x1, y2);
						painter.drawImage(layout.getResources()
								.getTripletFeelNone8(), x2, y1);
					} else {
						TGTripletFeelPainter.paintTripletFeel8(painter, x1, y2,
								scale);
						TGTripletFeelPainter.paintTripletFeelNone8(painter, x2,
								y1, scale);
					}
				} else if (previous == TGMeasureHeader.TRIPLET_FEEL_SIXTEENTH) {
					if (layout.isBufferEnabled()) {
						painter.drawImage(layout.getResources()
								.getTripletFeel16(), x1, y2);
						painter.drawImage(layout.getResources()
								.getTripletFeelNone16(), x2, y1);
					} else {
						TGTripletFeelPainter.paintTripletFeel16(painter, x1,
								y2, scale);
						TGTripletFeelPainter.paintTripletFeelNone16(painter,
								x2, y1, scale);
					}
				}
				...
				if (layout.isBufferEnabled()) {
					painter.drawImage(layout.getResources()
							.getTripletFeelNone8(), x1, y1);
					painter.drawImage(layout.getResources().getTripletFeel8(),
							x2, y2);
				} else {
					TGTripletFeelPainter.paintTripletFeelNone8(painter, x1, y1,
							scale);
					TGTripletFeelPainter.paintTripletFeel8(painter, x2, y2,
							scale);
				}
			} else if (getTripletFeel() == TGMeasureHeader.TRIPLET_FEEL_SIXTEENTH) {
				if (layout.isBufferEnabled()) {
					painter.drawImage(layout.getResources()
							.getTripletFeelNone16(), x1, y1);
					painter.drawImage(layout.getResources().getTripletFeel16(),
							x2, y2);
				} else {
					TGTripletFeelPainter.paintTripletFeelNone16(painter, x1,
							y1, scale);
					TGTripletFeelPainter.paintTripletFeel16(painter, x2, y2,
							scale);
				}
			}
		}
	...
	private void paintMarker(ViewLayout layout, TGPainter painter) {
		if (this.hasMarker()) {
			...
			int y = (getPosY() + getTs().getPosition(
					TGTrackSpacing.POSITION_MARKER));
	...
	private void paintPlayMode(ViewLayout layout, TGPainter painter) {
		if (layout.isPlayModeEnabled() && isPlaying(layout)) {
			...
			if ((style & (ViewLayout.DISPLAY_SCORE | ViewLayout.DISPLAY_TABLATURE)) == (ViewLayout.DISPLAY_SCORE | ViewLayout.DISPLAY_TABLATURE)) {
				y1 += (getTs().getPosition(
						TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) - layout
						.getScoreLineSpacing());
				y2 += (getTs().getPosition(TGTrackSpacing.POSITION_TABLATURE)
						+ getTrackImpl().getTabHeight() + layout
						.getStringSpacing());
			} else if ((style & ViewLayout.DISPLAY_SCORE) != 0) {
				y1 += (getTs().getPosition(
						TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) - layout
						.getScoreLineSpacing());
				y2 += (getTs().getPosition(
						TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) + (layout
			...
			painter.initPath();
			painter.setAntialias(false);
			painter.addRectangle(getPosX() + (5f * scale), y1, width
					- (10f * scale), (y2 - y1));
			painter.closePath();
	...
	/**
	 * Retorna true si se esta reproduciendo y la posicion del player esta en
	 * este compas.
	 */
	...
	private int getMaxQuarterSpacing(ViewLayout layout) {
		return (((layout.getStyle() & ViewLayout.DISPLAY_MULTITRACK) != 0) ? getHeaderImpl()
				.getMaxQuarterSpacing() : this.quarterSpacing);
	}
	...
	public void setTs(TGTrackSpacing ts) {
		if (getTs() == null) {
			setBufferCreated(false);
		} else if (getTs().getPosition(
				TGTrackSpacing.POSITION_SCORE_MIDDLE_LINES) != ts
	...
	public Color getMarkerColor() {
		...
		if (this.markerColor == null || this.markerColor.isDisposed()) {
			this.markerColor = new Color(TuxGuitar.instance().getDisplay(),
	...
	public String toString() {
		return "TGMeasureImpl [getClef()=" + getClef() + ", getKeySignature()="
				+ getKeySignature() + ", getStart()=" + getStart()
				+ ", getTempo()=" + getTempo() + ", getTimeSignature()="
				+ getTimeSignature() + ", getLength()=" + getLength()
				+ ", getMarker()=" + getMarker() + "]";
	}
}
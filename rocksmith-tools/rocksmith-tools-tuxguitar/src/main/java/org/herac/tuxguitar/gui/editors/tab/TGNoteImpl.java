...
public class TGNoteImpl extends TGNote {
	...
	private final Rectangle noteOrientation;
	...
	private void paintOfflineEffects(ViewLayout layout, TGPainter painter,
		...
		if (effect.isAccentuatedNote()) {
			...
			int y = (bsY + bs
					.getPosition(TGBeatSpacing.POSITION_ACCENTUATED_EFFECT));
		...
		if (effect.isHarmonic()
			...
			int x = fromX + getPosX() + spacing;
			int y = (bsY + bs
					.getPosition(TGBeatSpacing.POSITION_HARMONIC_EFFEC));
			...
			key = effect.getHarmonic().isArtificial() ? TGEffectHarmonic.KEY_ARTIFICIAL
					: key;
			key = effect.getHarmonic().isTapped() ? TGEffectHarmonic.KEY_TAPPED
					: key;
			key = effect.getHarmonic().isPinch() ? TGEffectHarmonic.KEY_PINCH
					: key;
			key = effect.getHarmonic().isSemi() ? TGEffectHarmonic.KEY_SEMI
					: key;
		...
		if (effect.isSlapping()) {
			int x = fromX + getPosX() + spacing;
			int y = (bsY + bs
					.getPosition(TGBeatSpacing.POSITION_SLAPPING_EFFEC));
		...
		if (effect.isPalmMute()) {
			int x = fromX + getPosX() + spacing;
			int y = (bsY + bs
					.getPosition(TGBeatSpacing.POSITION_PALM_MUTE_EFFEC));
	...
	public void paintTablatureNote(ViewLayout layout, TGPainter painter,
		...
		if ((style & ViewLayout.DISPLAY_TABLATURE) != 0) {
			...
			layout.setTabNoteStyle(
					painter,
					(layout.isPlayModeEnabled() && getBeatImpl().isPlaying(
							layout)));
			// -------------ligadura--------------------------------------
			if (isTiedNote() && (style & ViewLayout.DISPLAY_SCORE) == 0) {
				...
				if (noteForTie != null) {
					tX = (fromX + noteForTie.getPosX() + noteForTie
							.getBeatImpl().getSpacing());
					tY = (fromY + noteForTie.getTabPosY() + stringSpacing);
					tWidth = (x - tX);
				} else {
					Rectangle r = layout
							.getNoteOrientation(painter, x, y, this);
			...
			if (!layout.isPlayModeEnabled()) {
				...
				if ((style & ViewLayout.DISPLAY_SCORE) == 0) {
					int y1 = (fromY
							+ getMeasureImpl().getTrackImpl().getTabHeight() + (stringSpacing / 2));
					int y2 = (fromY
							+ getMeasureImpl().getTrackImpl().getTabHeight() + ((stringSpacing / 2) * 5));
	 ...
	 */
	private void paintScoreNote(ViewLayout layout, TGPainter painter,
			int fromX, int fromY, int spacing) {
		if ((layout.getStyle() & ViewLayout.DISPLAY_SCORE) != 0) {
			...
			// -------------foreground--------------------------------------
			boolean playing = (layout.isPlayModeEnabled() && getBeatImpl()
					.isPlaying(layout));
			...
			if (isTiedNote()) {
				...
				if (noteForTie != null) {
					int tNoteX = (fromX + noteForTie.getPosX() + noteForTie
							.getBeatImpl().getSpacing());
			...
			if (this.accidental == TGMeasureImpl.NATURAL) {
				...
				painter.initPath(TGPainter.PATH_FILL);
				TGKeySignaturePainter.paintSharp(painter,
						(x - (scale - (scale / 4))), (y1 + (scale / 2)), scale);
				painter.closePath();
			} else if (this.accidental == TGMeasureImpl.FLAT) {
				painter.initPath(TGPainter.PATH_FILL);
				TGKeySignaturePainter.paintFlat(painter,
						(x - (scale - (scale / 4))), (y1 + (scale / 2)), scale);
				painter.closePath();
			}
			// ----------fin sostenido--------------------------------------
			if (getEffect().isHarmonic()) {
				if (layout.isBufferEnabled()) {
					painter.drawImage(
							layout.getResources().getHarmonicNote(
									getVoice().getDuration().getValue(),
									playing), x, y1);
				} else {
					boolean full = (getVoice().getDuration().getValue() >= TGDuration.QUARTER);
					painter.initPath((full ? TGPainter.PATH_DRAW
							| TGPainter.PATH_FILL : TGPainter.PATH_DRAW));
					TGNotePainter.paintHarmonic(painter, x, y1 + 1,
							(layout.getScoreLineSpacing() - 2));
					painter.closePath();
				}
			} else {
				if (layout.isBufferEnabled()) {
					painter.drawImage(
							layout.getResources().getScoreNote(
									getVoice().getDuration().getValue(),
									playing), x, y1);
				} else {
					boolean full = (getVoice().getDuration().getValue() >= TGDuration.QUARTER);
					painter.initPath((full ? TGPainter.PATH_FILL
							: TGPainter.PATH_DRAW));
					TGNotePainter.paintNote(
							painter,
							x,
							y1 + 1,
							((full ? layout.getScoreLineSpacing() + 1 : layout
			...
			if (!layout.isPlayModeEnabled()) {
				...
				if (getVoice().getDuration().isDotted()
					...
					getVoiceImpl().paintDot(layout, painter,
							(x + (12.0f * (scale / 8.0f))),
							(y1 + (layout.getScoreLineSpacing() / 2)),
							(scale / 10.0f));
				}
				...
				if (getVoice().getDuration().getValue() >= TGDuration.HALF) {
					...
					if (getEffect().isStaccato()) {
						...
						painter.addOval(sX - (size / 2), sY - (size / 2), size,
								size);
						painter.closePath();
					}
					// tremolo picking
					if (getEffect().isTremoloPicking()) {
						...
						if ((direction == TGBeatGroup.DIRECTION_UP)) {
							tpY += (getVoiceImpl().getBeatGroup().getMaxNote()
									.getScorePosY()
									- layout.getScoreLineSpacing() - 4);
						} else {
							tpY += (getVoiceImpl().getBeatGroup().getMinNote()
									.getScorePosY()
									+ layout.getScoreLineSpacing() + 4);
						}
						for (int i = TGDuration.EIGHTH; i <= getEffect()
					...
					if (getEffect().isStaccato()) {
						int size = 3;
						int sX = x
								+ (layout.getResources().getScoreNoteWidth() / 2);
						int sY = (fromY
								+ getVoiceImpl().getBeatGroup().getMinNote()
										.getScorePosY() + layout
								.getScoreLineSpacing()) + 2;
						layout.setScoreEffectStyle(painter);
						painter.initPath(TGPainter.PATH_FILL);
					...
					if (getEffect().isTremoloPicking()) {
						layout.setScoreEffectStyle(painter);
						painter.initPath();
						int tpX = (x + (layout.getResources()
								.getScoreNoteWidth() / 2));
						int tpY = fromY
								+ (getVoiceImpl().getBeatGroup().getMaxNote()
										.getScorePosY()
										- layout.getScoreLineSpacing() - 4);
						for (int i = TGDuration.EIGHTH; i <= getEffect()
								.getTremoloPicking().getDuration().getValue(); i += i) {
		...
		for (int i = getMeasureImpl().countBeats() - 1; i >= 0; i--) {
			...
			if (beat.getStart() < getBeatImpl().getStart()
				...
				while (it.hasNext()) {
					...
					if (note.getString() == getString()) { return note; }
				}
			}
		}
		return null;
	...
	private void paintEffects(ViewLayout layout, TGPainter painter, int fromX,
		...
		if (effect.isGrace()) {
			...
			painter.drawString(value,
					(this.noteOrientation.x - gracePoint.x - 2),
					this.noteOrientation.y);
		}
		if (effect.isBend()) {
			...
			int nextFromX = fromX;
			TGNoteImpl nextNote = (TGNoteImpl) layout
					.getSongManager()
					.getMeasureManager()
					.getNextNote(getMeasureImpl(), getBeatImpl().getStart(),
							getVoice().getIndex(), getString());
			if (effect.isSlide()) {
	...
	private void paintTremoloBar(ViewLayout layout, TGPainter painter,
			int fromX, int fromY) {
		float scale = layout.getScale();
	...
	private void paintHammer(ViewLayout layout, TGPainter painter,
		...
		float width = (nextNote != null) ? ((nextNote.getPosX() + nextFromX + nextNote
				.getBeatImpl().getSpacing()) - (4.0f * xScale) - x)
				: (10.0f * xScale);
	...
	private void paintVibrato(ViewLayout layout, TGPainter painter, int fromX,
		...
		if (loops > 0) {
			...
			for (int i = 0; i < loops; i++) {
				...
				painter.cubicTo((x + (2.0f * scale)), (y + (1.0f * scale)),
			...
			painter.cubicTo((x + (7.0f * scale)), (y + (2.0f * scale)),
					(x + (8.0f * scale)), (y + (2.0f * scale)),
					(x + (7.0f * scale)), (y + (3.0f * scale)));

			for (int i = (loops - 1); i >= 0; i--) {
				x = (fromX + ((6.0f * scale) * i));
				painter.lineTo((x + (6.0f * scale)), (y + (4.0f * scale)));
				painter.cubicTo((x + (6.0f * scale)), (y + (4.0f * scale)),
						(x + (5.0f * scale)), (y + (5.0f * scale)),
						(x + (4.0f * scale)), (y + (4.0f * scale)));
	...
	public String toString() {
		return "TGNoteImpl [getValue()=" + getValue() + ", getVelocity()="
				+ getVelocity() + ", getString()=" + getString()
				+ ", isTiedNote()=" + isTiedNote() + ", getEffect()="
				+ getEffect() + "]";
	}
}
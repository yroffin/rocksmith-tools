...
public class MasterPlayer implements RealtimeSoundPlayer, EventSoundPlayer {
	...
	public Performance arrange(Song song, Arrangement passedArrangement) {
		...
		Performance performance = soundPlayer.createPerformance(tracks,
				song.getTempo(), song.getResolution());
		...
		for (int m = 0; m < measures; m++) {
			...
			if ((lastTimeSignature == null)
					|| (!currentTimeSignature.equals(lastTimeSignature))) {
		...
		for (int t = 1; t <= tracks; t++) {
			...
			for (int v = 0; v < virtualTracks; v++) {
				...
				for (int m = 0; m < measures; m++) {
					Integer measureIndex = (Integer) arrangedMeasures.get(m);
					SongMeasure measure = song
							.getPerformanceMeasure(measureIndex.intValue());

					SongMeasureTrack smt = measure.getTrack(songTrack);
					SongVirtualTrack svt = smt.getVirtualTrack(v);
					Collection<SongEvent> events = svt.getEvents();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see players.sound.SoundPlayer#createPerformance(int,
	 * adaptors.song.Tempo, int)
	 */

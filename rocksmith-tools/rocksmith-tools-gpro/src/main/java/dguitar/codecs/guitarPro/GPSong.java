...
public class GPSong {
	public GPSong() {
		...
		chordsDiagrams = new ArrayList<Object>(0);
		copyright = "";
		instruction = "";
		interpret = "";
		key = GPKey.G;
	...
	class StringImpl {
		...
		StringImpl(int howDeep) {
			this.tabsStrinImpl = 0;
			this.deep = howDeep;
		}

		/**
		 * Modifies the RES variable and Append to the ScrollTextPanel as
		 * necesary the number of tabs added depend on the tabsStrinImpl
		 * variable.
		 */
		...
		public String toString() {
			...
			if (deep > 1) {
				...
				for (i = 0; i < measuresTracksPairs.size(); i++) {
					resAppend("Measure #" + ((i / numTracks) + 1) + " Track #"
							+ (i % numTracks) + "\n");
		 ...
		 */
		/*
		 * public int getLengthOfTask() { lengthOfTask = 10 ;
		 * 
		 * return lengthOfTask; }
		 */
		 ...
		 */
		/*
		 * public void stop() { canceled = true; statMessage = null; }
		 */
		 ...
		 */
		/*
		 * public String getMessage() { return statMessage; }
		 */
	}
	...
	public GPMeasureTrackPair getTrackMeasurePair(int track, int measure) {
		return measuresTracksPairs.get((measure - 1) * tracks.size() + track
				- 1);
	}

	public void startDisplayString() {
	}
	...
	public String toString() {
		...
		// return res ;
		return new StringImpl(100).toString();
	}

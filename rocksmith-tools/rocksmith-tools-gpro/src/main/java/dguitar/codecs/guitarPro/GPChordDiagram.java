...
public class GPChordDiagram {
	...
	/**
	 * The root of the Diagram (-1 for the customized chords, 0 when root is C,1
	 * = C#...)
	 */
	private GPChordNote root;

	/**
	 * Determines the chord type as followed: - 0: M - 1: 7 - 2: 7M - 3: 6 - 4:
	 * m - 5: m7 - 6: m7M - 7: m6 - 8: sus2 - 9: sus4 - 10: 7sus2 - 11: 7sus4 -
	 * 12: dim - 13: aug - 14: 5
	 */
	private GPChordType chordType;

	/**
	 * Nine, Eleven of Thirteen: byte Determines if the chord goes until the
	 * ninth, the eleventh, or the thirteenth.
	 */
	...
	/**
	 * Diminished/Augmented Integer Tonality linked with 9/11/13: 0: perfect
	 * ("juste") 1: augmented 2: diminished
	 */
	private GPTonalityType tonalityType;

	/**
	 * add: byte Allows to determine if a 'add' (added note) is present in the
	 * chord.
	 */
	...
	/**
	 * Fifth: byte Tonality of the fifth: 0: perfect ("juste") 1: augmented 2:
	 * diminished
	 */
	private GPTonalityType tonalityFive;

	/**
	 * Ninth: byte Tonality of the ninth: 0: perfect ("juste") 1: augmented 2:
	 * diminished this tonality is valid only if the value
	 * "Nine, Eleven or Thirteen" is 11 or 13.
	 */
	private GPTonalityType tonalityNine;

	/**
	 * Eleventh: byte Tonality of the eleventh: 0: perfect ("juste") 1:
	 * augmented 2: diminished this tonality is valid only if the value
	 * "Nine, Eleven or Thirteen" is 13.
	 */
	...
	/**
	 * Frets: List of 7 integers. Corresponds to the fret number played on each
	 * string, from the highest to the lowest. -1 means a string unplayed. 0
	 * means a string played "blank" (ie no fret).
	 */
	private final int[] frets;

	/**
	 * Number of barres: byte Indicates the number of barres there are in the
	 * chord. A chord can contain up to 5 barres.
	 */
	private int numBarres;

	/**
	 * Fret of the barre: List of 5 Bytes Indicates the fret number of each
	 * possible barre.
	 */
	private final int[] fretOfBarres;

	/**
	 * Barre start: List of 5 Bytes Indicates the first string of the barre, 1
	 * being the highest. The list order matches the chord different barres
	 * frets list order.
	 */

	private final int[] barreStarts;

	/**
	 * Barre end: List of 5 Bytes Indicates the first string of the barre, 1
	 * being the lowest. The list order matches the chord different barres frets
	 * list order.
	 */
	private final int[] barreEnds;

	/**
	 * Fingering: List of 7 Bytes Describes the fingering used to play the
	 * chord. Below is given the fingering from the highest string to the
	 * lowest: -2: unknown; -1: X or 0 (no finger); 0: thumb; 1: index; 2:
	 * middle finger; 3: annular; 4: little finger.
	 */
	private final GPFingering[] fingering;
	...
	public GPChordDiagram() {
		...
		setChordFingeringDisplayed(false);
	}
	...
	public void setchordFingeringDisplayed(boolean value) {
		this.setChordFingeringDisplayed(value);
	}
	...
	public boolean isChordFingeringDisplayed() {
		return chordFingeringDisplayed;
	}

	public void setChordFingeringDisplayed(boolean chordFingeringDisplayed) {
		this.chordFingeringDisplayed = chordFingeringDisplayed;
	}
}
/*
 * root = 0 ; chordType = null ; nineElevenThirteen = 0 ; bass = 0 ;
 * tonalityType = null ; addedNote = 0 ; name = "" ; tonalityFive = null ;
 * tonalityNine = null ; tonalityEleven = null ; baseFret = 0 ; strings = new
 * boolean[7] ; numBarres = 0 ; fretsOfBarres = new int [5] ; barreStarts = new
 * int [5] ; barreEnds = new int [5] ; fingering = null ; chorFingeringDisplayed
 * = false ;
 */

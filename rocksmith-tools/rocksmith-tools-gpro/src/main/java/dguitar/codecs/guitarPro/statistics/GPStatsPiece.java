...
public class GPStatsPiece extends GPStats {
	...
	/**
	 * this class gathers information about a single GPSong, you can dedicde if
	 * you want detailed, The Sring pieceName contains the name to display.
	 */
	public void getStats(GPSong p, String PieceName) {
		...
		MTPtotal = new GPStatsMeasureTrackPair();
		if (this.isDetailed()) {
			MTPdetails = new Vector<GPStatsMeasureTrackPair>(0, 1);
		}
		for (i = 0; i < numMTPs; i++) {
	...
	public String toString() {
		...
		/*
		 * are there usefull statistics about number of repetitions, time
		 * signatures ?? for (i = 0; i < numMeasures ; i++) {
		 * resAppend("\n\tMeasure #" + i) ; resAppend("\n\t" +
		...
		/*
		 * for (i = 0; i < numTracks ; i++) { resAppend("\n\tTrack #" + i) ;
		...
		res += "\t"
				+ this.stringFor(numTracksWithLyrics, "track with lyrics",
						numTracks, "tracks");
		if (MTPtotal != null) {
			level++;
		...
		/*
		 * resAppend("\n\tMIDI Channels - BEGIN"); for (i = 0; i < 4; i++) {
		 * resAppend("\n\t\tPort: " + i ); for (j = 0; j < 16; j++) {
		 * resAppend("\n\t\t\tChannel: " + j); resAppend("\n\t\t\t" +
		 * channels[i*16+j]) ; } } resAppend("\n\tMIDI Channels - END");
		 * 
		 * 

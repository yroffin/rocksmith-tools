 ...
 */
public class Midi implements LicenseString {
	...
	private java.util.List<MidiEvent> eventList;
	 ...
	 */
	private final String doesNotHaveAnyReceivers;
		...
		if (currentReceiver != null) {
			...
			while (playing && (it.hasNext())) {
				...
				try {
					...
					Util.showDialogOk(null, "IllegalStateException",
	...
	/**
	 * This method sets the instrument and clears the current sequence setting
	 * it to 16 PPQs
	 */
	...
	/**
	 * returns the next index available..if end of device list is reached it
	 * returns 0
	 * 
	 * @param i
	 *            the current index
	...
	private static int midiMessageLength(int status) {
		...
		case 0x00F0: // realtime and sysex
			if (status == ShortMessage.SONG_SELECT) return 2;
			if (status >= ShortMessage.TUNE_REQUEST) return 1;
		default:
			return 3;
		}
	}
	...
	private static String MidiMessageToString(MidiMessage MM) {
		...
		if (MM instanceof ShortMessage) {
			...
			msg = new byte[3];
			msg[0] = (byte) status;
			if (length > 1) msg[1] = (byte) sm.getData1();
			if (length > 2) msg[2] = (byte) sm.getData2();
		} else {
			msg = MM.getMessage();
			length = MM.getLength();
		}
	...
	public Vector<MidiTrackEvent> SequenceToMidiTrackEvents() {
		...
		V = new Vector<MidiTrackEvent>(0, 1);
		tracks = this.sequence.getTracks();
